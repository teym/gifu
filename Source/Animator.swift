...
class Animator: NSObject {
    ...
    private var originImageSource:CGImageSource!
    ...
    required init(data: NSData, delegate: Animatable) {
        originImageSource = CGImageSourceCreateWithData(data, nil)
    ...
    private func prepareFrames(imageSource: CGImageSourceRef, size: CGSize) {
        (animatedFrames, totalDuration) = updateFrames(imageSource, size: size, filter: {$0})
    }
    private func updateFrames(imageSource: CGImageSourceRef, size: CGSize,filter:(CGImage) -> CGImage) -> ([AnimatedFrame],NSTimeInterval) {
        let numberOfFrames = Int(CGImageSourceGetCount(imageSource))
        
        return reduce(0..<numberOfFrames, ([AnimatedFrame](), 0.0)) { accumulator, index in
            ...
            let frameImageRef = filter(CGImageSourceCreateImageAtIndex(imageSource, index, nil))
